...
**XState + Lit + mcss | Single-file HTML | Event-driven | Pure JavaScript | Custom Signaling Server**
...
- Хранилище данных
- Сетевое взаимодействие (включая конфигурируемый сигнальный сервер)
- Медиа-обработка
...
// Пример: метрики сигнального сервера
metrics.gauge(
	"signaling.server",
...
### 2. StorageService (с батчингом и миграцией настроек)

Уже описан выше в разделе "Performance & Backpressure". Добавим поддержку конфигурируемого сигнального сервера:
...
### 3. SignalingService (с поддержкой кастомного URL)
...
class SignalingService {
	...
	// Проверка, используется ли дефолтный сервер
...
### Компонент настройки сигнального сервера
...
### 4. Yandex Cloud Function API Requirements
...
│  │  ├─ groups.machine.js
│  │  ├─ streams.machine.js
│  │  ├─ settings.machine.js         ← конфигурация сигнального сервера
│  │  ├─ modal.machine.js
│  │  ├─ notification.machine.js
│  │  ├─ persistence.machine.js
│  │  ├─ shell.machine.js
│  │  ├─ lifecycle.machine.js
│  │  ├─ error-boundary.machine.js
│  │  └─ instrumentation.machine.js
│  │
│  ├─ services/
│  │  ├─ crypto.service.js
│  │  ├─ storage.service.js
│  │  ├─ signaling.service.js        ← с поддержкой кастомного URL
│  │  └─ media.service.js
│  │
│  ├─ runtime/
│  │  ├─ event-bus.js
...
│  │  ├─ streams/
│  │  ├─ settings/
│  │  │  └─ signaling-settings.js    ← UI для кастомного сервера
│  │  ├─ modals/
│  │  └─ profile/
│  │
│  ├─ styles/
...
- **Services** = доменная логика (включая конфигурируемый сигнальный сервер)
...
- Загрузка настроек сигнального сервера при boot
- Ленивая загрузка
- Performance budgets

### 7. Конфигурируемый Сигнальный Сервер

- Дефолтный сервер (наш)
- Возможность указать кастомный URL Yandex Cloud Function
...
7. ✅ **Гибкость инфраструктуры** - поддержка кастомных сигнальных серверов

---

## 🔧 Workflow использования кастомного сигнального сервера
...
3. AppMachine → applyingSettings → configureSignaling()
4. signalingService.setCustomURL(url)
5. Connect to custom server ✅
```

---

## 📊 Дополнительные метрики для кастомного сервера
...
metrics.increment("signaling.switch", 1, { from: "default", to: "custom" });
```

---

## 🎯 Итоговый чеклист возможностей

### Основной функционал

- ✅ XState FSM архитектура
- ✅ Actor Model с lifecycle management
- ✅ Priority Event Bus
- ✅ Worker-based криптография
- ✅ Worker-based медиаобработка
- ✅ Batching для IndexedDB
- ✅ Error Boundary с recovery strategies
- ✅ Performance monitoring
- ✅ Startup optimization (COLD/WARM/REHYDRATE/OFFLINE)
...
- ✅ Конфигурируемый сигнальный сервер
- ✅ Дефолтный сервер (готовый к использованию)
- ✅ Возможность кастомного Yandex Cloud Function
- ✅ API совместимость
...
- ✅ Safe mode при критических ошибках
...
1. **WebRTC P2P** - прямые соединения между клиентами
2. **Media Streaming** - video/audio calls
3. **Federation** - подключение к другим серверам/сетям
4. **Plugin System** - расширяемость через плагины
5. **Advanced Search** - полнотекстовый поиск по сообщениям
6. **Bots API** - интеграция с ботами
7. **Voice Messages** - запись и отправка голосовых сообщений

### Масштабирование

- **Sharding** - разделение данных по серверам
- **Load Balancing** - балансировка нагрузки между экземплярами
- **Replication** - репликация данных для отказоустойчивости
- **CDN Integration** - доставка медиа через CDN

---

## 📝 Заключение

Architecture v0.3 представляет собой production-ready решение для децентрализованного мессенджера с фокусом на:

1. **Надёжность** через изоляцию ошибок и стратегии восстановления
2. **Производительность** через worker offloading и thread budgeting
3. **Масштабируемость** через actor model и modular design
4. **Гибкость** через конфигурируемую инфраструктуру
5. **Observability** через comprehensive monitoring и logging

Архитектура готова к deployment как single-file HTML приложения с возможностью использования как дефолтного сигнального сервера, так и собственного развёртывания на Yandex Cloud Functions.

Все компоненты следуют принципам чистой архитектуры, что обеспечивает лёгкость тестирования, поддержки и дальнейшего развития.
